...
{
    var movie=movieModel.aggregate([{"$lookup":{ from: 'moviereviews', localField: 'title', foreignField: 'title', as: 'reviews' }},{"$unset":["username","reviews.username"]},{"$match":movieDetail}],(err,doc)=>
    {
        return res.status(200).json({"success":true, "movies":doc});
    });
}
